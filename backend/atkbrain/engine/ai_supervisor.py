...
- must_intents / next_plan 是参考假说：循环不会强制从者逐条执行。局面（未消费凭证、未关输入面、已验证洞、跳板/邻题、点名的那一条 hop_auth）由循环按攻击图编译，从者必须守。局面点名的过门 Intent 不是参考假说，本轮必须经已有壳/SSRF/转发去走。不要把「先打另一个允许 POST 的路由」写成换通道。假说最多 3 条正交，供从者选用。违背局面（入口枚举、离开未关输入面、不走过门局面）才会收紧禁令；其余 must 改打正交假说不算违约。空字段由循环按图补全。
- 调度层不会每轮都问你：从者整轮打完、且当前方案卡住或到了周期才开口。被问到时，简报里本轮认领的 Intent 仍开放 → hold=true 表示继续当前局面（可并行子智能体），禁止另起同义散文或改打目录枚举。
- CTF：唯一目标是正确 flag。题面给出的账号、路径、文件优先于自造字典；交旗优先于把题审完。本题必有解：禁止 rockyou / 超过 10 万行的词表 / hashcat 全库去撞哈希或登录；个位数默认口令须打在认证处理接口上，未送达不算失败；送达后失败不要升级字典。经开放代理/SSRF 打内层登录时，外壳 POST 或只回登录页不是口令否证。RCE / getshell / 橙线高亮都是手段。图上的边权乘积（常见自动补边 0.55）不是夺旗概率，禁止当进度或收工信号。评测 request_hint 会扣分：先打活体；实在卡住（活体、题面路径、一手利用都空转）才允许 next_plan 点名 request_hint；开局禁止。已看过不要再点。平台总分差不是漏旗。
...
9. 范围：本机网卡和物机网关是守卫，不是目标。邻题入口（同评测其它 unique_code 的入口 IP/端口）禁止当本题主线。本题立足点/已验证 SSRF 看见的 RFC1918 是本题内网，不是邻题——只把打出业务面的邻机扩进 Scope，再经跳板打（shell / SSRF 载荷，或立足点隧道 + 本机 ssh/sshpass）；禁止无跳板 Kali 直连、禁止当邻题换址、禁止打攻击机 docker 网桥。单个入口地址不授权同网段邻居。本题入口端口不通时 rebind_entry=true，不要改打邻题端口。公网文档域名可以访问。CTF/红队/SRC 从者均可 WebSearch：已识别产品/版本时 next_plan 应要求查 CVE/N-day，不要当断网。禁止用 unique_code/题名搜 writeup。
...
13. 多 flag 未齐且已有 ★GETSHELL/已验证 SSRF：本机已交过的旗不要再挖。入口 webshell/SSRF 是跳板，不要关掉。端口转发/代理映射不等于已经登上邻机。邻机是新身份域：must_intents 三条必须不同 tactic，同一 tactic 只占一格。身份验证与未授权可达并行，一个失败不关闭另一个，也不把「先过门」写成其它面的前置条件。经跳板打内层登录：先确认认证处理接口收到方法与请求体；外壳 POST、站点根路径、仍回登录页 HTML 都不是口令否证，不能结束 hop_auth，也不能因此换用户、扫目录或回头打入口注入。同一身份面在认证接口已送达后没有新秘密、只重复失败 → 该 hop_auth 做完，换其它 tactic。
...
- 禁止破坏性写入；SQLi 只用读证明。红队/SRC：改密/支付/竞争/轰炸只证明不落地。
- 不要打本机控制台端口或攻击机网卡 IP。
...
_SRC_CHAIN_MUST = (
    ...
    "不要把暂缓类型硬塞进 must。禁止三格都加深同一条洞，禁止 socks/shell 横向与 GETSHELL 收工。"
    "已验证 SSRF 须 report_pivot_capability。hold=false。"
)
...
_ENUM_SRC = (
    ...
    "HTTP 活体：must 至少一格是建议测里的打洞（web_inject/越权/逻辑/N-day 等），subagents 含 src-hunt 与 recon 即可，不要再叠一个 web-exploit。"
    "禁止三格都是 fingerprint / protocol_model / content_enum。静态 SPA 不是没有攻击面。"
    "next_plan / must_intents 禁止提权、report_flag、GETSHELL 收工、Kali 直连 RFC1918。"
    "已验证 SSRF 必须 report_pivot_capability 扩网，禁止把本站当跳板却不登记。"
    "禁止开局 -p- 或超 10 万行词表。"
)
